...
// Error
func (self *CryptError) Error() string {
	return fmt.Sprintf("CryptError: ErrCode=%d, ErrMsg=%s", self.ErrCode, self.ErrMsg)
}
...
func (self *WXBizMsgCrypt) DecryptRawMsg(msg_signature, timestamp, nonce string, encrypt string) ([]byte, *CryptError) {
	signature := self.calSignature(timestamp, nonce, encrypt)

	if strings.Compare(signature, msg_signature) != 0 {
		return nil, NewCryptError(ValidateSignatureError, "signature not equal")
	}

	plaintext, crypt_err := self.cbcDecrypter(encrypt)
	if nil != crypt_err {
		return nil, crypt_err
	}
